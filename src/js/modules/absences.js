...
function checkAndShowMessageModal() {
  const messageModal = document.getElementById("messageModals");
...
function initModals() {
  document.querySelectorAll(".close-modals").forEach((btn) => {
    btn.addEventListener("click", function () {
      const modal = this.closest(
        "#agentModals, #deleteModals, #qrModals, #messageModals"
      );
      if (modal) {
        closeModal(modal.id);
      }
  ...
  document
    .querySelectorAll("#agentModals, #deleteModals, #qrModals, #messageModals")
    .forEach((modal) => {
...
function setupListeners() {
  document.body.addEventListener("click", function (e) {
    ...
    if (target.matches(".add-agent-btns")) {
      addAgent();
    }

    // Boutons d’édition
    if (
      target.matches(".edit-agent-btns") ||
      target.closest(".edit-agent-btns")
    ) {
      const btn = target.matches(".edit-agent-btns")
        ? target
        : target.closest(".edit-agent-btns");
      const agentId = btn.getAttribute("data-ids");
      editAgent(agentId);
    }

    // Boutons de génération QR
    if (target.matches(".qr-agent-btns") || target.closest(".qr-agent-btns")) {
      const btn = target.matches(".qr-agent-btns")
        ? target
        : target.closest(".qr-agent-btns");
      const agentId = btn.getAttribute("data-ids");
      generateQR(agentId);
    }
    ...
    if (
      target.matches(".delete-agent-btns") ||
      target.closest(".delete-agent-btns")
    ) {
      const btn = target.matches(".delete-agent-btns")
        ? target
        : target.closest(".delete-agent-btns");
      const agentId = btn.getAttribute("data-ids");
  ...
  const downloadQRBtn = document.getElementById("downloadQRBtns");
  if (downloadQRBtn) {
    downloadQRBtn.addEventListener("click", function () {
      const qrCanvas = document
        .getElementById("qrCodeContainers")
        .querySelector("canvass");
      if (qrCanvas) {
        const qrAgentName = document.getElementById("qrAgentNames").textContent;
...
export function addAgent() {
  const modal = document.getElementById("agentModals");
  if (!modal) return;
  document.getElementById("modalTitles").innerHTML =
    '<i class="fas fa-user-plus mr-2 text-indigo-600"></i><span>Ajouter un nouvel agent</span>';
  const form = document.getElementById("agentForm");
  if (form) {
    form.reset();
    document.getElementById("agent_ids").value = "";
    document.getElementById("actions").value = "add";
  }
  showModal("agentModals");
}
...
export function editAgent(agentId) {
  ...
  const form = document.getElementById("agentForms");
  if (!form) return;
...
export function generateQR(agentId) {
  ...
  const modal = document.getElementById("qrModals");
  if (!modal) {
...
export function confirmDelete(agentId) {
  const modal = document.getElementById("deleteModals");
  if (!modal) return;
  ...
  if (confirmDeleteBtn) {
    confirmDeleteBtn.href = `?page=absence_content&action=delete&id=${agentId}`;
  }

  showModal("deleteModals");
...
function setupFilters() {
  ...
  const agentsTableBody = document.querySelector("#agentsTables tbody");
  ...
  function filterAndDisplayAgents() {
    ...
    const tableContainer = agentsTableBody.closest("#agentsTables");
