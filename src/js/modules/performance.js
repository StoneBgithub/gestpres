...
export function init() {
    ...
    fetch("fetch_services.php")
        ...
        .then(services => {
            ...
            // Pré-remplir le service si défini dans l'URL
        ...
        if (serviceId) {
            fetch(`fetch_bureaux.php?service_id=${serviceId}`)
                ...
                .then(bureaux => {
                    ...
                    // Pré-remplir le bureau si défini dans l'URL
                    const urlParams = new URLSearchParams(window.location.search);
                    if (urlParams.get("bureau_id")) {
                        bureauSelect.value = urlParams.get("bureau_id");
                    }
                })
                .catch(error => {
            ...
            .then(data => {
                ...
                if (!data.agents || data.agents.length === 0) {
                    ...
                    data.agents.forEach((agent, index) => {
                        ...
                        const presenceRate = Math.round(agent.presence_rate || 0);
                        ...
                        const lateCount = agent.late_count || 0;
                        const earlyDepartureCount = agent.early_departure_count || 0;
                        ...
                        agentsRanking.innerHTML += `
                            <tr class="hover:bg-gray-50 transition-all">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 font-medium">${Math.round(agent.total_hours || 0)} h</div>
                                    <div class="text-xs text-gray-500">+${Math.round(agent.overtime_hours || 0)} h supp.</div>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">${lateCount} retards / ${earlyDepartureCount} départs anticipés</div>
                                </td>
                            </tr>
                        `;
                    });
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg shadow-sm border-l-4 border-congo-green">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-congo-green">${data.stats.total_hours || 0} h</p>
                                </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg shadow-sm border-l-4 border-congo-green">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-congo-green">${data.stats.avg_attendance_rate || 0}%</p>
                                </div>
                                <div class="rounded-full bg-congo-green-light p-3">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg shadow-sm border-l-4 border-congo-green">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-congo-green">${data.stats.total_overtime || 0} h</p>
                                </div>
                                <div class="rounded-full bg-congo-green-light p-3">
                                    <i class="fas fa-hourglass-half text-congo-green text-xl"></i>
                                </div>
                            </div>
                            <p class="text-xs text-gray-500 mt-2">Après 14h00</p>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg shadow-sm border-l-4 border-congo-green">
                            <div class="flex justify-between items-center">
                                <div>
                                    <p class="text-sm font-medium text-gray-500">Agents à 100% de présence</p>
                                    <p class="text-2xl font-bold text-congo-green">${data.stats.perfect_attendance || 0}</p>
                                </div>
                                <div class="rounded-full bg-congo-green-light p-3">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg shadow-sm border-l-4 border-congo-green">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-congo-green">${data.stats.total_late || 0} / ${data.stats.total_early_departure || 0}</p>
                                </div>
                                <div class="rounded-full bg-congo-green-light p-3">
                                    <i class="fas fa-exclamation-triangle text-congo-green text-xl"></i>
                                </div>
                            </div>
                            <p class="text-xs text-gray-500 mt-2">Arrivées > 08h00 / Départs < 14h00</p>
                        </div>
                    `;
                }
    ...
    exportBtn.addEventListener("click", () => {
        ...
        fetch(`fetch_performances.php?${params.toString()}`)
            ...
            .then(data => {
                const csv = [
                    ...
                    ...data.agents.map((agent, index) =>
                        [
                            ...
                            `"${agent.bureau || "-"}"`,
                            `"${agent.service || "-"}"`,
                            Math.round(agent.total_hours || 0),
                            Math.round(agent.overtime_hours || 0),
                            Math.round(agent.presence_rate || 0),
                            agent.late_count || 0,
                            agent.early_departure_count || 0
                        ].join(",")
                    )
                ].join("\n");
                ...
                a.href = url;
                a.download = `performances_agents_${new Date().toISOString().split('T')[0]}.csv`;
                a.click();
