...
function setupFilters() {
 ...
 const tableContainer = agentsTableBody.closest("#compteTable");
...
function setupFormSubmission() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    fetch("http://localhost/gestpres/gestpres/src/ajax_content.php", {
      method: "POST",
      body: formData
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          ...
          eventBus.publish("comptes:externalUpdate", {}); // recharge les données
        } else {
