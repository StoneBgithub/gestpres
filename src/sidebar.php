...
// Récupérer le nombre de nouvelles actions non vues
try {
    ...
    error_log("Erreur dans sidebar.php : " . $e->getMessage());
    $nbnouvelles = 0; // fallback
}
?>
...
<aside class="w-64 bg-white shadow-xl border-r border-gray-100">
    <div class="h-full flex flex-col">
        ...
        <!-- Navigation Menu -->
        <nav class="flex-1 py-4 px-2" aria-label="Menu de navigation">
            <ul class="space-y-1">
                <?php
                $role = $_SESSION['role'] ?? 'secrétaire'; 
                ...
                if ($role === 'admnistrateur') {
                    $menu_items['compte_content'] = [
                        'title' => 'Gestions des utilisateurs',
                        'icon' => 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z'
                    ];
                    $menu_items['parametrage_content'] = [
                        'title' => 'Parametrage du système',
                        'icon' => 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z'
                    ];
                }
                ...
                foreach ($menu_items as $page_name => $item) {
                    ...
                    echo '<li>';
                    echo '<a href="dashboard.php?page=' . htmlspecialchars($page_name) . '" class="sidebar-menu flex items-center px-4 py-3 rounded-xl hover:bg-congo-yellow-pale hover:text-congo-green-dark transition-all duration-300 card-shine ' . $active_class . '" data-page="' . htmlspecialchars($page_name) . '" aria-label="' . htmlspecialchars($item['title']) . '">';
                    ...
                    // Badge uniquement pour "historique_content"
                    if ($page_name === 'historique_content' && $nbnouvelles > 0) {
                        echo '<span id="notification-badge" class="badge-rond" aria-label="Nouvelles actions non vues : ' . $nbnouvelles . '" aria-live="polite">' . $nbnouvelles . '</span>';
                    }
        ...
        <div class="p-4 mt-auto border-t border-gray-200">
            <a href="logout.php"
                class="flex items-center text-red-600 hover:bg-red-100 hover:text-red-800 transition-all duration-300 rounded-xl px-4 py-3 card-shine"
