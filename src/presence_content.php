...
<body>
    <div class="bg-gray-50 p-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div class="flex flex-wrap gap-2">
                <button id="toggle-all"
                    class="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-gray-200 transition"
                    data-type="all">Tous</button>
                <button id="toggle-arrivals"
                    class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition"
                    data-type="arrivée">Arrivées</button>
                <button id="toggle-departures"
                    class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition"
                    data-type="depart">Départs</button>
                <div class="relative flex items-center">
                    <a href="#" id="toggle-filters-btn"
                        class="px-3 py-1.5 bg-white border border-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-congo-yellow-pale hover:text-congo-green-dark transition flex items-center">
                        Filtres avancés
                    </a>
                    <span id="filter-indicator"
                        class="ml-2 inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-gradient-to-tr from-green-600 to-green-900 text-white shadow-sm transition-all duration-300 <?php echo $filtersApplied ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'; ?>">
        ...
        <!-- Filtres avancés -->
        <div id="advanced-filters" class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 mb-6"
            style="display: none;">
            <form id="filter-form" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="date" id="date" name="date" value="<?php echo htmlspecialchars($date); ?>"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3">
                    </div>
                    <div>
                        <label for="time-range" class="block text-sm font-medium text-gray-700 mb-1">Plage
                            horaire</label>
                        <select id="time-range" name="time_range"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3">
                            <option value="all" <?php echo $time_range === 'all' ? 'selected' : ''; ?>>Toute la journée
                            </option>
                            <option value="morning" <?php echo $time_range === 'morning' ? 'selected' : ''; ?>>Matin
                                (8h-12h)</option>
                            <option value="afternoon" <?php echo $time_range === 'afternoon' ? 'selected' : ''; ?>>
                                Après-midi (12h-18h)</option>
                            <option value="custom" <?php echo $time_range === 'custom' ? 'selected' : ''; ?>>
                                Personnalisé...</option>
                        </select>
                        <div id="custom-time" class="mt-2"
                            style="display: <?php echo $time_range === 'custom' ? 'block' : 'none'; ?>;">
                            <input type="time" name="custom_start" id="custom_start"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3 mb-2"
                                value="<?php echo htmlspecialchars($custom_start); ?>">
                            <input type="time" name="custom_end" id="custom_end"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3"
                                value="<?php echo htmlspecialchars($custom_end); ?>">
                        </div>
                    </div>
                    <div>
                        ...
                        <select id="status" name="status"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3">
                            <option value="all" <?php echo $status === 'all' ? 'selected' : ''; ?>>Tous les statuts
                            </option>
                            <option value="on-time" <?php echo $status === 'on-time' ? 'selected' : ''; ?>>À l'heure
                            </option>
                            <option value="late" <?php echo $status === 'late' ? 'selected' : ''; ?>>En retard</option>
                            <option value="early" <?php echo $status === 'early' ? 'selected' : ''; ?>>Départ anticipé
                            </option>
                        </select>
                    </div>
                </div>
                <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select id="service" name="service"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3">
                            <option value="all" <?php echo $service === 'all' ? 'selected' : ''; ?>>Tous les services
                            </option>
                            <?php foreach ($services as $svc): ?>
                            <option value="<?php echo htmlspecialchars($svc['libele']); ?>"
                                <?php echo $service === $svc['libele'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($svc['libele']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="bureau" name="bureau"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3">
                            <option value="all" <?php echo $bureau === 'all' ? 'selected' : ''; ?>>Tous les bureaux
                            </option>
                            <?php foreach ($bureaux as $bur): ?>
                            <option value="<?php echo htmlspecialchars($bur['libele']); ?>"
                                <?php echo $bureau === $bur['libele'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($bur['libele']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="employee" name="employee"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3">
                            <option value="all" <?php echo $employee === 'all' ? 'selected' : ''; ?>>Tous les employés
                            </option>
                            <?php foreach ($agents as $agt): ?>
                            <option value="<?php echo htmlspecialchars($agt['nom_prenom']); ?>"
                                <?php echo $employee === $agt['nom_prenom'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($agt['nom_prenom']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="mt-6 flex justify-end space-x-3">
                    <button type="button" id="reset-filters"
                        class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition">Réinitialiser</button>
                    <button type="submit" id="apply-filters"
                        class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition">Appliquer
                        les filtres</button>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <form action="export_presence_csv.php" method="POST" class="inline">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-gradient-to-tr from-green-600 to-green-900 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition">
                        Exporter en CSV
                    </button>
                </form>
            </div>
            <div class="overflow-x-auto">
                <table id="presence-table" class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr
                            class="hover:bg-gray-50 text-sm entry-row <?php echo $presence['type'] === 'arrivée' ? 'arrival' : 'departure'; ?>">
                            <td class="px-6 py-4">
                                <?php 
                            ...
                            <td class="px-6 py-4 font-medium"><?php echo htmlspecialchars($presence['nom_prenom']); ?>
                            </td>
                            ...
                            <td class="px-6 py-4 min-w-[180px]">
                                ...
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap <?php echo $presence['heure'] > '09:00:00' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'; ?>">
                                    ...
                                    <span
                                        class="truncate"><?php echo $presence['heure'] > '09:00:00' ? 'Arrivée (Retard)' : 'Arrivée'; ?></span>
                                </span>
                                <?php else: ?>
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap <?php echo $presence['heure'] < '17:00:00' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'; ?>">
                                    <svg class="w-3.5 h-3.5 mr-1 flex-shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <span
                                        class="truncate"><?php echo $presence['heure'] < '17:00:00' ? 'Départ anticipé' : 'Départ'; ?></span>
                                </span>
                                <?php endif; ?>
                            </td>
                        </tr>
