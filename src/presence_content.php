...
$arrivalsStmt = $pdo->prepare("SELECT COUNT(*) FROM presence p JOIN agent a ON p.agent_id = a.id JOIN bureau b ON a.bureau_id = b.id JOIN service s ON b.service_id = s.id $where_clause AND p.type = 'arrivée'");
$arrivalsStmt->execute($params);
$arrivals = $arrivalsStmt->fetchColumn();

$departuresStmt = $pdo->prepare("SELECT COUNT(*) FROM presence p JOIN agent a ON p.agent_id = a.id JOIN bureau b ON a.bureau_id = b.id JOIN service s ON b.service_id = s.id $where_clause AND p.type = 'depart'");
$departuresStmt->execute($params);
$departures = $departuresStmt->fetchColumn();

$lateStmt = $pdo->prepare("SELECT COUNT(*) FROM presence p JOIN agent a ON p.agent_id = a.id JOIN bureau b ON a.bureau_id = b.id JOIN service s ON b.service_id = s.id $where_clause AND p.type = 'arrivée' AND p.heure > '09:00:00'");
...
$bureaux_json = json_encode($bureaux);
$agents_json = json_encode($agents);
?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div id="custom-time" class="mt-2" style="display: <?php echo $time_range === 'custom' ? 'block' : 'none'; ?>;">
                        <input type="time" name="custom_start" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3 mb-2" value="<?php echo htmlspecialchars($custom_start); ?>">
                        <input type="time" name="custom_end" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-white py-2 px-3" value="<?php echo htmlspecialchars($custom_end); ?>">
                    </div>
                </div>
                <div>
                            ...
                            <td class="px-6 py-4">
                                ...
                                if ($photoExists): ?>
                                    <img src="<?php echo htmlspecialchars($presence['photo']); ?>" 
                                        ...
                                        loading="lazy"
                                        onerror="this.parentNode.innerHTML = getInitialsCircle('<?php echo htmlspecialchars($presence['nom_prenom']); ?>')">
                                <?php else: ?>
...
<script>
    ...
    const bureaux = <?php echo $bureaux_json; ?>;
